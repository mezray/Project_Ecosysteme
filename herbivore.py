...
class Herbivore(Vivant):
    ...
    def inRayonContact(self,cible):#si une cible est dans le rayon de contact
        if len(listeDesHerbivores) > 7500:
            oui = 0
        else:
            oui = 0
        ...
        if str(cible.__class__.__bases__[0])=="<class 'plante.Plante'>" and oui == 0:#si un carnivore croise un herbivore, herbivore sera manger
    ...
    def deplacer(self,directionNumber):#fonction permet de se déplacer suivant une direction: ("S","SW","W","NW","N","NE","E","SE")
        ...
        while i<len(ListedesVivants):
            if self.inZone(self.rayonContact,ListedesVivants[i].position)==True:#regarde s'il y a qqch dans le rayon de contact
                self.inRayonContact(ListedesVivants[i])
            if self.inZone(self.rayonVision,ListedesVivants[i].position)==True:#regarde s'il y a qqch dans le rayon de vision
                return self.inRayonVision(ListedesVivants[i])
            i+=1
